...
class Task_Assigner {
  ...
  async #prepare() {
    ...
    const workload = Math.floor(this.#series_list_number / this.#threadNumber);
  ...
  async #getThreadsData(threadId, threadData) {
    ...
    const threadSeriesData = await this.#getThreadSeriesData(threadId);
    return { ...threadConfig, seriesData: threadSeriesData };
  }
  ...
  async #getThreadSeriesData(id) {
    const PATH = `${WORKER_MAIN_DIR_PATH}/${id}/${WORKER_SERIES_DATA_FILE_NAME}`;
    try {
      const configs = await fs.readFile(PATH);
      return JSON.parse(configs.toString());
    } catch (err) {
      if (err.code === "ENOENT") {
        return await this.#createDefaultThreadSeriesDataFile(id);
      }
    }
  }

  async #createDefaultThreadSeriesDataFile(id) {
    const PATH = `${WORKER_MAIN_DIR_PATH}/${id}/${WORKER_SERIES_DATA_FILE_NAME}`;
    const data = {};
...
async function saveNewThreadExpertData(threadId, newData) {
  const PATH = `${WORKER_MAIN_DIR_PATH}/${threadId}/${WORKER_SERIES_DATA_FILE_NAME}`;
  try {
