...
class Worker {
  ...
  constructor(task) {
    ...
    this.#end = task.end;
    this.seriesData = task.seriesData;
  }
  ...
  async #scrap() {
    try {
      ...
      const SERIES_URL = process.env["SERIES_LIST_URL"];

      const SERIES_LIST_FULL_URL = `https://${BASE_URl}/${SERIES_URL}`;
      ...
      const path = (p) => `${SERIES_LIST_FULL_URL}/page/${p}`;
      ...
      for (let n = this.#progress; n < this.#end + 1; n++) {
        ...
        for (const link of postsLink) {
          ...
          this.seriesData[link.postName] = anchors;

          // save new expert data
          await saveNewThreadExpertData(this.#workerId, this.seriesData);
